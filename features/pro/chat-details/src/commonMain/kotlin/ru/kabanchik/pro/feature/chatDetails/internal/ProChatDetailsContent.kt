...
import kabanchik.features.pro.chat_details.generated.resources.Res
import kabanchik.features.pro.chat_details.generated.resources.chat_details_hint
...
internal fun ProChatDetailsContent(
    ...
    when (state) {
        ProChatDetailsContract.State.Loading -> {
            CommonScreenLoader()
        }
        is ProChatDetailsContract.State.Chat -> {
            Chat(
                state = state,
...
@Composable
private fun Chat(
    state: ProChatDetailsContract.State.Chat,
    onMessageTextChanged: (String) -> Unit,
    onMessageSent: () -> Unit
) {
